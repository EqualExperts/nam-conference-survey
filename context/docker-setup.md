# NAM Conference Survey - Docker Setup
...
```bash
# =============================================================================
# NAM Conference Survey - Environment Variables
# =============================================================================
...
# -----------------------------------------------------------------------------
# Password for admin dashboard access (default: admin123)
...
**Network**: `nam-survey-network` (bridge driver)
...
docker-compose exec postgres psql -U postgres -d nam_survey
```
...
docker volume rm nam-conf-survey_postgres_data
```
...
services:
  ...
  backend:
    ...
    environment:
      NODE_ENV: production
      # Load secrets from secret manager (GCP Secret Manager)
...
- Google OAuth restricted to localhost
...
# Specific containers
docker stats nam-survey-backend nam-survey-frontend
```

### Logs Aggregation (Future)
- Integrate with ELK stack (Elasticsearch, Logstash, Kibana)
- Or use cloud provider logging (GCP Cloud Logging)
